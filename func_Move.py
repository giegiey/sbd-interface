...
# ------------------------------
        ...
        #timeout after no response then proceed
        diffTime = datetime.now() - var_global.startTime
        if ((diffTime.seconds*1000) + (diffTime.microseconds/1000)) >= sTimeout:
            print(datetime.now().strftime("%H:%M:%S.%f") +
                  " Move 11 : Timeout finish cancel and conveyor stop ")
            GPIO.output(ConFWD, GPIO.HIGH)
            var_global.iStep = 99
...
# ------------------------------
        ...
        if var_global.bBCchecking == False:
            if var_global.bBCpass == True:
                var_global.bBCpass = True
                print(datetime.now().strftime("%H:%M:%S.%f") +
                    " Move 10 : Conveyor Forward ")
                GPIO.output(ConFWD, GPIO.LOW)
                GPIO.output(ConREV, GPIO.HIGH)
                var_global.startTime = datetime.now()
                var_global.iStep += 1
            else:
                print(datetime.now().strftime("%H:%M:%S.%f") + " Move 10 : Barcode Server Check Fail ")
                var_global.nRetry = 0
                var_global.iStep = 5
...
# ------------------------------
        ...
        if ((diffTime.seconds*1000) + (diffTime.microseconds/1000)) >= sTimeout:
            print(datetime.now().strftime("%H:%M:%S.%f") +
                  " Move 11 : Timeout finish so proceed ")
            var_global.iStep += 1
        # check bag pass thru output sensor
        # check output sensor for clear
        # input/output sensor 1: clear 0: block
        if GPIO.input(OutputSen) == 1:
            print(datetime.now().strftime("%H:%M:%S.%f") +
                  " Move 11 : Check output sensor for clear ")
            var_global.iStep += 1

# ------------------------------
    elif var_global.iStep == 12:
        diffTime = datetime.now() - var_global.startTime
        if ((diffTime.seconds*1000) + (diffTime.microseconds/1000)) >= tBagMoveCont:
            GPIO.output(ConFWD, GPIO.HIGH)
            print(datetime.now().strftime("%H:%M:%S.%f") +
                  " Move 12 : Conveyor stop ")
            var_global.iStep = 100
...
# Error no response
        ...
        GPIO.output(LEDgreen, GPIO.HIGH)
        var_global.bBlinkRed = True
        var_global.SocketTxMsg1 = "002,003,004\n"
        var_global.iStep += 1

# ------------------------------
# Error intrusion sensor
    elif var_global.iStep == 99:  # Intrusion red light alert
        GPIO.output(LEDred, GPIO.LOW)
        GPIO.output(LEDgreen, GPIO.HIGH)
        var_global.bBlinkRed = True
